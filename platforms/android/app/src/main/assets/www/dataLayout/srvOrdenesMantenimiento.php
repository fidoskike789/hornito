<?php
    ...
    function listarOrdenesMantenimiento(){
		...
		if (isset($_GET["data"]))
		{
			...
			$consulta = "SELECT om.idOrdenMantenimiento, om.idFlujo, om.cliente, om.telefono,
								...
								om.ciudad, om.garantia, om.fechaRegistro, om.usuarioRegistro, om.estado,
								(
									select c.detalle
    ...
    function insertarOrdenMantenimiento(){
		...
		if (isset($_GET["data"]))
		{
			...
			$sql = $dbConn->prepare("INSERT into ft_ordenmantenimiento (idflujo, cliente, telefono,
									 ...
									 VALUES (:f, :c, :t, :d, :fe, :s, :fm, :p, :fv, :tm, :tr, :dsc, :g, :ciu, :fa, :u, 1)");
			...
			$sql->bindValue(':ciu', utf8_decode($data->{'ciu'}));
    ...
    function buscarVisitaTecnica(){
		...
		if (isset($_GET["data"]))
		{
			...
			$consulta = "SELECT vt.idVisitaTecnica, vt.idOrdenMantenimiento, vt.maquina, vt.serie, vt.modelo, vt.costoMantenimiento, om.garantia,
						 		vt.fechaRegistro, vt.usuario
                         from ft_visitatecnica vt
                         join ft_ordenmantenimiento om
                         on vt.idOrdenMantenimiento = om.idOrdenMantenimiento
                         where vt.idOrdenMantenimiento = :idOM and vt.estado = 1";
    ...
    function insertarVisitaTecnica(){
		...
		if (isset($_GET["data"]))
		{
			...
			$consulta = "INSERT into ft_visitatecnica (idOrdenMantenimiento, maquina, serie, modelo, estadocodigo, fechaRegistro, usuario, estado)
						 values (:idOM, :m, :s, :md, :ec, :fr, :u, 1)";
    ...
    function actualizarVisitaTecnica(){
		...
		if (isset($_GET["data"]))
		{
			...
			/*$consulta = "UPDATE ft_visitatecnica set maquina = :m, serie = :s, modelo = :md, costoMantenimiento = :c, garantia = :g
						 where idOrdenMantenimiento = :idOM and estado = 1";*/
			$consulta = "UPDATE ft_visitatecnica set maquina = :m, serie = :s, modelo = :md, estadocodigo = :ec
						 where idOrdenMantenimiento = :idOM and estado = 1";
    ...
    function guardarObservaciones(){
		...
		if (isset($_GET["data"]))
		{
			...
			$sql = $dbConn->prepare("UPDATE ft_visitatecnica
										set costoMantenimiento = :c
									where idOrdenMantenimiento=:idOM");
			$sql->bindValue(':idOM', $data->{'idOM'});
			$sql->bindValue(':c', $data->{'c'});
			$sql->execute();
			...
			$bitacora = [
			    'idOrdenMantenimiento' => $data->{'idOM'},
			    'idFlujo' => 6,
			    'accion' => 'Observaciones y costo registrados, orden Nº: '.$data->{'idOM'}.'. Observaciones: '.$data->{'o'},
			    'usuario' => $data->{'u'}
			]; insertarBitacora($bitacora);
